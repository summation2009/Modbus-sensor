...
void loop()
{
  ...
  if (result == myModbus.ku8MBSuccess) {               // หากสำเร็จ เซนเซอร์ตอบกลับ และไม่มีผิดพลาด
    ...
    CO2 = myModbus.getResponseBuffer(5);                     // เอาค่า Buffer 5 ที่อ่านจาก Modbus มาไว้ในตัวแปร CO2
    Illumination = myModbus.getResponseBuffer(7);            // เอาค่า Buffer 7 ที่อ่านจาก Modbus มาไว้ในตัวแปร Illumination
    ...
    Serial.print (" *C");
    Serial.print ("\t");
    Serial.print (CO2);                   // นำค่าที่อ่านได้ พิมพ์ออกทาง Serial0
    Serial.print (" ppm");
    Serial.print ("\t");
    Serial.print (Illumination);          // นำค่าที่อ่านได้ พิมพ์ออกทาง Serial0
    Serial.println (" LUX");
