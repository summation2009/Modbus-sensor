...
void loop()
{
  ...
  if (result == myModbus.ku8MBSuccess) {               // หากสำเร็จ เซนเซอร์ตอบกลับ และไม่มีผิดพลาด
    UV_intensity = myModbus.getResponseBuffer(0) / 100;           // เอาค่า Buffer 0 ที่อ่านจาก Modbus มาไว้ในตัวแปร UV intensity หารด้วย 100 
    UV_index = myModbus.getResponseBuffer(1);                     // เอาค่า Buffer 1 ที่อ่านจาก Modbus มาไว้ในตัวแปร UV index

    Serial.print (UV_intensity);              // นำค่าที่อ่านได้ พิมพ์ออกทาง Serial0
    Serial.print(" mW/cm2");
    Serial.print ("\t");
    Serial.print (UV_index);              // นำค่าที่อ่านได้ พิมพ์ออกทาง Serial0
    Serial.print(" mW/cm2");
